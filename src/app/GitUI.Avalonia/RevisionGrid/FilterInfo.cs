...
public record FilterInfo
{
    ...
    private void GetCommitFilterSummary(StringBuilder filter)
    {
        if (ByDateFrom)
        {
            filter.AppendLine($"{"Since"}: {DateFrom}");
        }

        if (ByDateTo)
        {
            filter.AppendLine($"{"Until"}: {DateTo}");
        }
    }
    ...
    private void GetLimitingFilterSummary(StringBuilder filter)
    {
        ...
        if (ByPathFilter)
        {
            filter.AppendLine($"{"Path Filter"}: {PathFilter}");
        }

        if (ByAuthor && !string.IsNullOrWhiteSpace(Author))
        {
            filter.AppendLine($"{"Author"}: {Author}");
        }

        if (ByCommitter && !string.IsNullOrWhiteSpace(Committer))
        {
            filter.AppendLine($"{"Committer"}: {Committer}");
        }

        if (ShowSimplifyByDecoration)
        {
            filter.AppendLine($"{"Simplify by Decoration"}");
        }
        ...
        if (ByMessage && !string.IsNullOrEmpty(Message))
        {
            ...
            {
                filter.AppendLine($"{"Message"}: {Message}");
            }
        }

        if (ByDiffContent && !string.IsNullOrEmpty(DiffContent))
        {
            filter.AppendLine($"{"Diff Content"}: {DiffContent}");
        }
    }
    ...
    private void GetBranchFilterSummary(StringBuilder filter)
    {
        if (ShowOnlyFirstParent)
        {
            filter.AppendLine("Show Only First Parent");
        }

        if (ShowReflogReferences)
        {
            filter.AppendLine("Show Reflog");
        }

        if (IsShowCurrentBranchOnlyChecked)
        {
            filter.AppendLine("Show Current Branch Only");
        }
        else if (!string.IsNullOrWhiteSpace(BranchFilter))
        {
            filter.AppendLine($"{"Branches"}: {BranchFilter}");
        }
    }
